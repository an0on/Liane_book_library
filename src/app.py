...
# Try to import the connection_string from your connect.py
try:
...
if menu_selection == "🏠 Home":
    ...
    st.write("Select a section from the menu on the left to continue.")
    ...
    st.title("Currently Borrowed Books")
    query = """
    ...
    """
    try:
        df_loans = pd.read_sql(query, con=engine)
        ...
        try:
            ...
            if not friends_df.empty:
                ...
                if action_friend == "Edit":
                    ...
                    if st.button("Yes, Delete Friend"):
                        try:
                            with engine.begin() as conn:
